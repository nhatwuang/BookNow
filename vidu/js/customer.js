...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (hotelList && danhSachKhachSan.length > 0) {
    // Trộn mảng khách sạn và lấy 4 phần tử đầu tiên
    const khachSanRandom = shuffleArray([...danhSachKhachSan]).slice(0, 3);

    hotelList.innerHTML = khachSanRandom
      .map(
        (h, i) => `
        <div class="hotel-item" data-type="khachsan" data-index="${i}">
          <img src="${h.anh}" alt="${h.ten}" />
          <h3>${h.ten}</h3>
          <p><b>Mô tả:</b> ${h.moTa}</p>
