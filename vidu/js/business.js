...
function hienThiPhuongTien(pt, index) {
  ...
  item.innerHTML = `
    ...
    <div class="vehicle-info">
      <h3 class="movie-title">${pt.loai}</h3> 
...
function hienThiKhachSan(ks, index) {
  ...
  item.innerHTML = `
    ...
    <div class="hotel-info">
      <h3 class="hotel-title">${ks.ten}</h3>
      <p>🛏️ ${ks.moTa}</p>
...
let danhSachVe = JSON.parse(localStorage.getItem("danhSachVe")) || [];

// Hàm hiển thị vé
function hienThiDanhSachVe() {
  ticketsListEl.innerHTML = ""; // Xóa nội dung cũ

  if (danhSachVe.length === 0) {
    ticketsListEl.innerHTML = "<p>Chưa có vé nào được đặt.</p>";
    return;
  }

  danhSachVe.forEach((ve, index) => {
    const veEl = document.createElement("div");
    veEl.classList.add("ticket-item");
    veEl.innerHTML = `
      <h3>Vé #${index + 1} - ${ve.loaiDichVu}</h3>
      <p>Khách hàng: ${ve.tenKhach}</p>
      <p>Ngày: ${ve.ngayDat} | Giờ: ${ve.gioDat}</p>
      <p>Số lượng vé: ${ve.soLuongVe}</p>
      <p>Tổng tiền: ${ve.tongTien.toLocaleString("vi-VN", {
        style: "currency",
        currency: "VND",
      })}</p>
    `;
    ticketsListEl.appendChild(veEl);
  });
}
